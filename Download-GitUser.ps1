...
'@
  ...
  Process {
    ...
    foreach ($gitu in $Userp2) {
      ...
      ### Get GitRepos
      $dlgit = ('{0}\{1}' -f $Path, $gitu)
      $null =	if (!(Test-Path -Path $dlgit)) { New-Item -ItemType Directory -Path $dlgit }
      ...
      $g1.clone_url | Invoke-ForEachParallel -Process {
        Start-Process -WorkingDirectory $dlgit -FilePath "$env:ProgramFiles\Git\bin\git.exe" -ArgumentList ('clone --recursive {0}' -f $_) -WindowStyle Hidden -Wait
